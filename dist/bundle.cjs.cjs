...
class Big {
    constructor(big) {
        switch (typeof big) {
            ...
            case "bigint":
                ...
                break;
            case "object":
                if (big instanceof buffer.Buffer || big instanceof Uint8Array) {
    ...
    toBuffer(option) {
        ...
        if (option === "fill") {
            ...
            return buffer.Buffer.from(new Int64.Uint64BE(this.toString()).toBuffer());
        }
...
class Float {
    ...
    toBuffer() {
        const b = buffer.Buffer.allocUnsafe(8);
...
class Bool {
    ...
    toBuffer() {
        return this.b ? buffer.Buffer.from([1]) : buffer.Buffer.from([0]);
    }
    get v() {
        return this.b;
    }
...
let TimeStamp$1 = class TimeStamp {
    ...
    toBuffer() {
        return buffer.Buffer.from(this.UTC());
    }
    toString() {
        return this.ISO();
    }
...
class FullTimeStamp extends TimeStamp$1 {
    ...
    toBuffer(option) {
        return buffer.Buffer.from(option === "super" ? super.UTC() : this.UTC());
    }
    ISO() {
        const iso = super.ISO();
        if (this.r) {
...
let Token$1 = class Token {
    ...
    toString() {
        return buffer.Buffer.from(this.s, "utf8").toString("base64");
    }
};
...
const sha256 = (msg) => buffer.Buffer.from(sha256$1(msg));
const sha3 = (msg) => buffer.Buffer.from(jsSha3.sha3_256.create().update(msg).digest());
const keccak256 = (msg) => buffer.Buffer.from(jsSha3.keccak256.create().update(msg).digest());
const getChecksum = (hex) => {
    const hexLower = hex.toLowerCase();
    const hash = keccak256(buffer.Buffer.from(hexLower, 'ascii')).toString('hex');
    let checksum = '';
...
class Key {
    ...
    toBuffer() {
        return buffer.Buffer.from(this.toString());
    }
    toString() {
...
class Keys {
    ...
    get checksum() {
        ...
        const hash = keccak256(buffer.Buffer.from(address, 'ascii')).toString('hex');
    ...
    toBuffer() {
        return buffer.Buffer.concat([
            buffer.Buffer.concat(this._keys.sort((a, b) => buffer.Buffer.compare(buffer.Buffer.from(a.toString()), buffer.Buffer.from(b.toBuffer()))).map(k => k.toBuffer())),
    ...
    toHintedObject() {
        ...
        return {
            ...
            hash: eHash.slice(24),
            keys: this._keys
                .sort((a, b) => buffer.Buffer.compare(buffer.Buffer.from(a.toString()), buffer.Buffer.from(b.toBuffer())))
...
const SortFunc = (a, b) => buffer.Buffer.compare(a.toBuffer(), b.toBuffer());
...
const isOpFact = (operation) => {
    ...
    const isHashValid = operation._hash instanceof Uint8Array ||
        (typeof buffer.Buffer !== "undefined" && buffer.Buffer.isBuffer?.(operation._hash));
    return (isIdValid &&
        isHintValid &&
        isFactValid &&
        isFactSignsValid &&
...
const privateKeyToPublicKey = (privateKey) => {
    let privateBuf;
    if (!buffer.Buffer.isBuffer(privateKey)) {
        ...
        privateBuf = buffer.Buffer.from(privateKey, "hex");
    }
    else {
...
const compress = (publicKey) => {
    ...
    const compressedPublicKey = buffer.Buffer.concat([
        buffer.Buffer.from([0x02 + (yCoordinate[yCoordinate.length - 1] % 2)]),
        xCoordinate,
    ]);
...
class BaseKeyPair {
    ...
    ethSign(msg) {
        ...
        const r = buffer.Buffer.from(signature.r.toArray());
        const s = buffer.Buffer.from(signature.s.toArray());
        const sigLength = 4 + r.length + s.length;
        const sigBuffer = buffer.Buffer.alloc(sigLength);
    ...
    ethVerify(sig, msg) {
        if (typeof sig === "string") {
            sig = buffer.Buffer.from(base58.decode(sig));
        }
        const rlen = new Big(sig.subarray(0, 4).reverse());
        const r = buffer.Buffer.alloc(rlen.v);
        ...
        const base = buffer.Buffer.from([48, sig.length, 2]);
        const buf = buffer.Buffer.alloc(sig.length + 2);
        base.copy(buf, 0, 0, 4);
        rlen.toBuffer().copy(buf, 3);
        r.copy(buf, 4);
        buffer.Buffer.from([2]).copy(buf, 4 + rlen.v);
        slen.toBuffer().copy(buf, 5 + rlen.v);
        s.copy(buf, 6 + rlen.v);
        return verify(buf, sha256(msg), getPublicKey(this.signer, true));
    }
    static K(seed) {
        seed = buffer.Buffer.from(base58.encode(sha3(buffer.Buffer.from(seed))));
...
class KeyPair extends BaseKeyPair {
    ...
    getSigner() {
        return buffer.Buffer.from(this.privateKey.noSuffix, "hex");
    }
    getPub() {
...
class BaseOperation {
    ...
    signWithSigType(sigType, keypair, node) {
        const getFactSign = (keypair, hash) => {
            ...
            return new GeneralFactSign(keypair.publicKey, keypair.sign(buffer.Buffer.concat([buffer.Buffer.from(this.id), hash, now.toBuffer()])), now.toString());
        };
        const getNodeFactSign = (node, keypair, hash) => {
            const now = TimeStamp$1.new();
            return new NodeFactSign(node.toString(), keypair.publicKey, keypair.sign(buffer.Buffer.concat([
                buffer.Buffer.from(this.id),
                node.toBuffer(),
                hash,
                now.toBuffer(),
            ])), now.toString());
    ...
    toBuffer() {
        ...
        return buffer.Buffer.concat([
            this.fact.hash,
            buffer.Buffer.concat(this._factSigns.map((fs) => fs.toBuffer())),
        ]);
    }
    toHintedObject() {
        const operation = {
...
class OperationFact extends Fact {
    ...
    toBuffer() {
        return buffer.Buffer.concat([
            ...
            buffer.Buffer.concat(this.items.map((i) => i.toBuffer())),
        ]);
    }
    toHintedObject() {
        return {
...
class CreateAccountItem extends CurrencyItem {
    ...
    toBuffer() {
        return buffer.Buffer.concat([
            this.keys.toBuffer(),
            buffer.Buffer.concat(this.amounts.sort(SortFunc).map(am => am.toBuffer())),
        ]);
    }
    toHintedObject() {
        return {
...
class UpdateHandlerFact extends Fact {
    ...
    toBuffer() {
        return buffer.Buffer.concat([
            ...
            this.contract.toBuffer(),
            this.currency.toBuffer(),
            buffer.Buffer.concat(this.handlers.sort(SortFunc).map(a => a.toBuffer())),
        ]);
    }
    toHintedObject() {
        return {
...
let MintFact$3 = class MintFact extends NodeFact {
    ...
    toBuffer() {
        return buffer.Buffer.concat([
            super.toBuffer(),
            buffer.Buffer.concat(this.items.map(it => it.toBuffer())),
        ]);
    }
    toHintedObject() {
        return {
...
class FixedFeeer extends Feeer {
    ...
    toBuffer() {
        return buffer.Buffer.concat([
            this.receiver.toBuffer(),
            this.amount.toBuffer(),
            this.exchangeMinAmount ? this.exchangeMinAmount.toBuffer() : buffer.Buffer.from([])
        ]);
    }
    toHintedObject() {
        const feeer = {
...
class Operation extends Generator {
    ...
    async send(operation, headers) {
        ...
        Assert.check(Config.OP_SIZE.satisfy(buffer.Buffer.byteLength(JSON.stringify(operation), 'utf8')), MitumError.detail(ECODE.OP_SIZE_EXCEEDED, `Operation size exceeds the allowed limit of ${Config.OP_SIZE.max} bytes.`));
...
class Currency extends Generator {
    ...
    batchTransfer(sender, receivers, currency, amounts) {
        ...
        return new BaseOperation(this.networkID, new TransferFact$4(TimeStamp$1.new().UTC(), sender, receivers.map((receiver, idx) => new TransferItem$3(receiver, [new Amount(currency, amounts[idx])]))));
    }
    /**
...
let RegisterModelFact$7 = class RegisterModelFact extends ContractFact {
    ...
    toBuffer() {
        return buffer.Buffer.concat([
            ...
            this.uri.toBuffer(),
            this.currency.toBuffer(),
            buffer.Buffer.concat(this.minterWhitelist.sort(SortFunc).map(w => w.toBuffer())),
        ]);
    }
    toHintedObject() {
        return {
...
let ApproveItem$2 = class ApproveItem extends NFTItem {
...
let TransferItem$2 = class TransferItem extends NFTItem {
...
class Signers {
    ...
    toBuffer() {
        return buffer.Buffer.concat([
            buffer.Buffer.concat(this.signers.sort(SortFunc).map(s => s.toBuffer())),
        ]);
    }
    toHintedObject() {
        return {
...
class NFT extends ContractGenerator {
    ...
    transfer(contract, sender, receiver, nftIdx, currency) {
        const fact = new TransferFact$3(TimeStamp$1.new().UTC(), sender, [
            new TransferItem$2(contract, receiver, nftIdx, currency)
        ]);
    ...
    multiTransfer(contract, sender, receiver, nftIdx, currency) {
        ...
        const items = Array.from({ length: receiver.length }).map((_, idx) => new TransferItem$2(contractsArray[idx], receiver[idx], nftIdx[idx], currency));
    ...
    approve(contract, sender, approved, nftIdx, currency) {
        return new BaseOperation(this.networkID, new ApproveFact$2(TimeStamp$1.new().UTC(), sender, [
            new ApproveItem$2(contract, approved, nftIdx, currency)
        ]));
    }
    /**
    ...
    multiApprove(contract, sender, approved, nftIdx, currency) {
        ...
        const items = Array.from({ length: approved.length }).map((_, idx) => new ApproveItem$2(contractsArray[idx], approved[idx], nftIdx[idx], currency));
...
class AddTemplateFact extends ContractFact {
    ...
    toBuffer() {
        return buffer.Buffer.concat([
            ...
            buffer.Buffer.from(this.templateName),
            this.serviceDate.toBuffer(),
            this.expirationDate.toBuffer(),
            this.templateShare.toBuffer(),
            this.multiAudit.toBuffer(),
            buffer.Buffer.from(this.displayName),
            buffer.Buffer.from(this.subjectKey),
            buffer.Buffer.from(this.description),
...
let RegisterModelFact$5 = class RegisterModelFact extends ContractFact {
    ...
    toBuffer() {
        return buffer.Buffer.concat([
            super.toBuffer(),
            buffer.Buffer.from(this.option),
            this.policy.toBuffer(),
            this.currency.toBuffer(),
        ]);
    }
...
class VoteFact extends DAOFact {
    ...
    toBuffer() {
        return buffer.Buffer.concat([
            super.toBuffer(),
            this.vote.v === 0 ? buffer.Buffer.from([0x00]) : this.vote.toBuffer(),
            this.currency.toBuffer(),
        ]);
    }
    toHintedObject() {
...
class Whitelist {
    ...
    toBuffer() {
        return buffer.Buffer.concat([
            ...
            buffer.Buffer.concat(this.accounts.sort(SortFunc).map(a => a.toBuffer())),
        ]);
    }
    toHintedObject() {
        return {
...
class IssueFact extends ContractFact {
    ...
    toBuffer() {
        return buffer.Buffer.concat([
            super.toBuffer(),
            buffer.Buffer.from(this.projectID),
...
let TransferItem$1 = class TransferItem extends TokenItem {
...
let ApproveItem$1 = class ApproveItem extends TokenItem {
...
let TransferFromFact$1 = class TransferFromFact extends OperationFact {
    constructor(token, sender, items) {
...
class Token extends ContractGenerator {
    ...
    transfer(contract, sender, currency, receiver, amount) {
        const item = new TransferItem$1(contract, receiver, amount, currency);
        return new BaseOperation(this.networkID, new TransferFact$2(TimeStamp$1.new().UTC(), sender, [item]));
    }
    /**
    ...
    multiTransfer(contract, sender, currency, receiver, amount) {
        ...
        const contractsArray = convertToArray(contract, receiver.length);
        const items = Array.from({ length: receiver.length }).map((_, idx) => new TransferItem$1(contractsArray[idx], receiver[idx], amount[idx], currency));
    ...
    transferFrom(contract, sender, currency, receiver, target, amount) {
        const item = new TransferFromItem$1(contract, receiver, target, amount, currency);
        return new BaseOperation(this.networkID, new TransferFromFact$1(TimeStamp$1.new().UTC(), sender, [item]));
    }
    /**
    ...
    multiTransferFrom(contract, sender, currency, receiver, target, amount) {
        ...
        const contractsArray = convertToArray(contract, receiver.length);
        const items = Array.from({ length: receiver.length }).map((_, idx) => new TransferFromItem$1(contractsArray[idx], receiver[idx], target[idx], amount[idx], currency));
        return new BaseOperation(this.networkID, new TransferFromFact$1(TimeStamp$1.new().UTC(), sender, items));
    }
    /**
    ...
    approve(contract, sender, currency, approved, amount) {
        const item = new ApproveItem$1(contract, approved, amount, currency);
        return new BaseOperation(this.networkID, new ApproveFact$1(TimeStamp$1.new().UTC(), sender, [item]));
    }
    /**
    ...
    multiApprove(contract, sender, currency, approved, amount) {
        ...
        const contractsArray = convertToArray(contract, approved.length);
        const items = Array.from({ length: approved.length }).map((_, idx) => new ApproveItem$1(contractsArray[idx], approved[idx], amount[idx], currency));
...
class Signer extends Generator {
    ...
    accSign(keypair, operation) {
        const now = TimeStamp$1.new();
        const fs = new GeneralFactSign(keypair.publicKey.toString(), keypair.sign(buffer.Buffer.concat([
            buffer.Buffer.from(this.networkID),
        ...
        const factSigns = operation.signs
            .map((s) => buffer.Buffer.concat([
            buffer.Buffer.from(s.signer),
            base58.decode(s.signature),
            new FullTimeStamp(s.signed_at).toBuffer("super"),
        ]));
        //.sort((a, b) => Buffer.compare(a, b))
        const msg = buffer.Buffer.concat([
            base58.decode(operation.fact.hash),
            buffer.Buffer.concat(factSigns),
        ]);
    ...
    nodeSign(keypair, operation, node) {
        ...
        const now = TimeStamp$1.new();
        const fs = new NodeFactSign(node, keypair.publicKey.toString(), keypair.sign(buffer.Buffer.concat([
            buffer.Buffer.from(this.networkID),
            nd.toBuffer(),
            base58.decode(operation.fact.hash),
            now.toBuffer(),
        ...
        const factSigns = operation.signs
            ...
            .sort((a, b) => buffer.Buffer.compare(a, b));
        const msg = buffer.Buffer.concat([
            base58.decode(operation.fact.hash),
            buffer.Buffer.concat(factSigns),
        ]);
    ...
    FillUserOpHash(userOperation) {
        ...
        const msg = buffer.Buffer.concat([
            buffer.Buffer.from(JSON.stringify(hintedExtension)),
            base58.decode(userOperation.fact.hash),
            buffer.Buffer.concat(userOperation.signs.map((s) => buffer.Buffer.concat([
                buffer.Buffer.from(s.signer),
                base58.decode(s.signature),
                new FullTimeStamp(s.signed_at).toBuffer("super"),
            ]))),
