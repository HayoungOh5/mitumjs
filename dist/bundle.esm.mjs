...
var PRESCRIPTION = {
    REGISTER_MODEL: {
        FACT: "mitum-prescription-register-model-operation-fact",
        OPERATION: "mitum-prescription-register-model-operation",
    },
    REGISTER_PRESCRIPTION: {
        FACT: "mitum-prescription-register-prescription-operation-fact",
        OPERATION: "mitum-prescription-register-prescription-operation",
    },
    USE_PRESCRIPTION: {
        FACT: "mitum-prescription-use-prescription-operation-fact",
        OPERATION: "mitum-prescription-use-prescription-operation",
    },
};
...
class PrescriptionFact extends ContractFact {
    constructor(hint, token, sender, contract, prescriptionHash, currency) {
...
class RegisterPrescriptionFact extends PrescriptionFact {
    constructor(token, sender, contract, prescriptionHash, prescribeDate, endDate, hospital, currency) {
        ...
        Assert.check(Config.PRESCRIPTION.HASH.satisfy(prescriptionHash.toString().length), MitumError.detail(ECODE.INVALID_FACT, `prescription hash out of range, should be between ${Config.PRESCRIPTION.HASH.min} to ${Config.PRESCRIPTION.HASH.max}`));
        Assert.check(Config.PRESCRIPTION.HOSPITAL.satisfy(hospital.toString().length), MitumError.detail(ECODE.INVALID_FACT, `hospital name out of range, should be between ${Config.PRESCRIPTION.HOSPITAL.min} to ${Config.PRESCRIPTION.HOSPITAL.max}`));
        this._hash = this.hashing();
    }
    toBuffer() {
        return Buffer.concat([
...
class UsePrescriptionFact extends PrescriptionFact {
    constructor(token, sender, contract, prescriptionHash, prepareDate, pharmacy, currency) {
        ...
        Assert.check(Config.PRESCRIPTION.PHARMACY.satisfy(pharmacy.toString().length), MitumError.detail(ECODE.INVALID_FACT, `pharmacy name out of range, should be between ${Config.PRESCRIPTION.PHARMACY.min} to ${Config.PRESCRIPTION.PHARMACY.max}`));
        this._hash = this.hashing();
    }
    toBuffer() {
        return Buffer.concat([
...
class Prescription extends ContractGenerator {
    constructor(networkID, api, delegateIP) {
        super(networkID, api, delegateIP);
    }
    /**
    ...
    registerModel(contract, sender, currency) {
        return new Operation$1(this.networkID, new RegisterModelFact(TimeStamp.new().UTC(), sender, contract, currency));
    }
    /**
     ...
     */
    registerPrescription(contract, sender, prescriptionHash, prescribeDate, endDate, hospital, currency) {
        new URIString(prescriptionHash, 'prescriptionHash');
        const fact = new RegisterPrescriptionFact(TimeStamp.new().UTC(), sender, contract, prescriptionHash, prescribeDate, endDate, hospital, currency);
     ...
     */
    usePrescription(contract, sender, prescriptionHash, prepareDate, pharmacy, currency) {
        new URIString(prescriptionHash, 'prescriptionHash');
        const fact = new UsePrescriptionFact(TimeStamp.new().UTC(), sender, contract, prescriptionHash, prepareDate, pharmacy, currency);
        return new Operation$1(this.networkID, fact);
    }
    /**
     ...
     */
    async getPrescription(contract, prescriptionHash) {
        Assert.check(this.api !== undefined && this.api !== null, MitumError.detail(ECODE.NO_API, "API is not provided"));
        Address.from(contract);
        new URIString(prescriptionHash, 'prescriptionHash');
