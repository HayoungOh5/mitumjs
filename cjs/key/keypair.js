...
class BaseKeyPair {
    ...
    btcSign(msg) {
        return Buffer.from(secp256k1.signSync((0, index_js_4.sha256)((0, index_js_4.sha256)(msg)), this.signer));
    }
    ethSign(msg) {
    ...
    btcVerify(sig, msg) {
        ...
        return secp256k1.verify(sig, (0, index_js_4.sha256)((0, index_js_4.sha256)(msg)), secp256k1.getPublicKey(this.signer));
    }
    ethVerify(sig, msg) {
        if (typeof sig === "string") {
            sig = Buffer.from(bs58_1.default.decode(sig));
        }
        const rlen = new index_js_1.Big(sig.subarray(0, 4).reverse());
        const r = Buffer.alloc(rlen.v);
        const rb = new index_js_1.Big(sig.subarray(4, 4 + rlen.v));
        rb.toBuffer().copy(r, rlen.v - rb.byteLen());
        const s = sig.subarray(4 + rlen.v);
        const slen = new index_js_1.Big(s.length);
        ...
        s.copy(buf, 6 + rlen.v);
        return secp256k1.verify(buf, (0, index_js_4.sha256)(msg), secp256k1.getPublicKey(this.signer, true));
    }
    static K(seed) {
        seed = Buffer.from(bs58_1.default.encode((0, index_js_4.sha3)(Buffer.from(seed))));
        index_js_5.Assert.check(40 <= seed.length, index_js_5.MitumError.detail(index_js_5.ECODE.INVALID_SEED, "seed length out of range"));
        seed = seed.subarray(0, 40);
        const N = secp256k1.CURVE.n - BigInt(1);
        let k = new index_js_1.Big(seed).big;
        k %= N;
        k += BigInt(1);
        return k;
    }
...
class KeyPair extends BaseKeyPair {
    constructor(privateKey) {
        super(pub_js_1.Key.from(privateKey));
    }
    getSigner() {
    ...
    getPub() {
        if (this.privateKey.type === "btc") {
            return new pub_js_1.Key(bs58_1.default.encode((0, eccrypto_js_1.getPublicCompressed)(Buffer.from(this.signer))) + index_js_3.SUFFIX.KEY.MITUM.PUBLIC);
        }
        const publickeyBuffer = (0, converter_js_1.privateKeyToPublicKey)("0x" + this.privateKey.noSuffix);
        return new pub_js_1.Key((0, converter_js_1.compress)(publickeyBuffer) + index_js_3.SUFFIX.KEY.ETHER.PUBLIC);
    }
    sign(msg) {
        if (this.privateKey.type === "btc") {
...
KeyPair.generator = {
    random(option) {
        ...
        if (option === "btc") {
            return new KeyPair(bs58_1.default.encode(Buffer.from((0, secure_random_1.default)(32, { type: "Uint8Array" }))) + index_js_3.SUFFIX.KEY.MITUM.PRIVATE);
        }
        //return new KeyPair(ethWallet.generate().getPrivateKeyString().substring(2) + SUFFIX.KEY.ETHER.PRIVATE)
        return new KeyPair(ethers_1.Wallet.createRandom().privateKey.substring(2) + index_js_3.SUFFIX.KEY.ETHER.PRIVATE);
    },
    fromPrivateKey(key) {
    ...
    fromSeed(seed, option) {
        option = option !== null && option !== void 0 ? option : "btc";
        index_js_5.StringAssert.with(seed, index_js_5.MitumError.detail(index_js_5.ECODE.INVALID_SEED, "seed length out of range"))
            .satisfyConfig(index_js_2.Config.SEED)
            .excute();
        if (option === "btc") {
            return new KeyPair(bs58_1.default.encode(secp256k1.utils.hexToBytes(BaseKeyPair.K(seed).toString(16))) + index_js_3.SUFFIX.KEY.MITUM.PRIVATE);
        }
        return new KeyPair(BaseKeyPair.K(seed).toString(16) + index_js_3.SUFFIX.KEY.ETHER.PRIVATE);
    }
};
